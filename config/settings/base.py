...
FORM_RENDERER = 'django.forms.renderers.TemplatesSetting' #更改查找组件模板的顺序：自定义到系统默认的模板
...
SESSION_ENGINE = 'django.contrib.sessions.backends.cached_db'  #用cached_db保存到数据库中且重启数据不会丢失
...
#MARKDOWNX_SERVER_CALL_LATENCY = 2000 #设置前端markdownx结果显示时间间隔 毫秒
...
CHANNEL_LAYERS = {
    "default": {
        ...
        "CONFIG": {
            "hosts": [f'{env("REDIS_URL", default="redis://192.168.57.129:6379")}/3', ],  # channel layers缓存使用Redis 3
        },
    },
}
