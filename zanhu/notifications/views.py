...
def notification_handler(actor, recipient, verb, action_object, **kwargs):
    ...
    :return:                None
    """
    if actor.username !=recipient.username and recipient.username == action_object.user.username:  # 只通知接收者，及recipient == 动作对象的作者
        key = kwargs.get('key', 'notification')  #默认是notification
        ...
        async_to_sync(channel_layer.group_send)('notifications', payload)  #payload：需要传递的消息，就是cosumer中的text_data
