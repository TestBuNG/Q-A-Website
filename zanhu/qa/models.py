        ...
        for obj in query:
            for tag in obj.tags.names():
                if tag not in tag_dict:
                    ...
                    tag_dict[tag] += 1
        return tag_dict.items()   #Python 字典(Dictionary) items() 函数以列表返回可遍历的(键, 值) 元组数组。
"""
...
class Vote(models.Model):
    ...
    class Meta:
        ...
        unique_together = ('user', 'content_type', 'object_id')  # 联合唯一键(用户对同一条数据只能操作一次)
        # SQL优化（给经常使用的语句进行联合索引）
...
class Question(models.Model):
    ...
    def get_downvoters(self):
        """反对的用户"""
        return [vote.user for vote in self.votes.filter(value=False)]   #self.votes.filter相当于Vote.objects.filter
